...
- 服务会把**整段对话**（system + 历史 + 工具结果）连同一小段协议说明序列化后发给 claude.ai，
  约定模型把工具调用当**纯文本** `<tool_call>{"name":...,"arguments":{...}}</tool_call>` 输出
  （并明确不要用 claude.ai 自带的代码执行 / artifact）。提示词只补「输出格式」这一层，任务说明
  由 Agent 客户端（如 Cursor）自己的 system prompt 提供。
- 从 completion 原始 SSE 拿到回答后解析回标准的 `tool_calls`（`finish_reason="tool_calls"`，流式同样支持）。
...
6. 启动 OpenAI 兼容 API（`serve.py`）：`POST /v1/chat/completions` 把消息填入 claude.ai 输入框，
   再用 `page.on("response")` 抓取 claude.ai **completion 接口的原始 SSE** 作为回答（不抓 DOM，
   保留 `<tool_call>` 标记），解析后**流式 SSE** 返回；仅一条 user 消息时自动开新对话；会话到
   `minutes` 后退出

排错：`GET /v1/debug/last` 返回最近一次提问「发出的问题 + claude.ai 返回的原始 SSE + 解析结果」；
日志里也会打印这些（完整内容另存到 `scripts/debug/`，Actions 会作为产物上传）。
